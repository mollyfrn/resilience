...
#changed working directory to Desktop folder 
setwd("C:/Users/MJenkins/Desktop/RadarPlotsRerun")
...
####Graphing plots for eqach city/community####
...
####Bar charts detailing rankings for each city/community####
#v1 self-written
setwd("C:/Users/MJenkins/Desktop/RadarPlotsRerun/NewBarPlots")
for(c in cities){
  ...
  data$id <- seq(1, nrow(data)) #ensures niches are clustered and consistently positioned
  
  p <- ggplot(data, aes(x=as.factor(id), y=nichecounts_minmaxnorm, fill=category)) +      
    geom_bar(aes(x=as.factor(id), y=nichecounts_minmaxnorm, fill=category), stat="identity", alpha=0.5) 
  p #02/03 so far so good, pretty straightforward to plot as bar 
 
  p+geom_bar(aes(x=as.factor(id), y=nichecounts_minmaxnorm, fill=category), stat="identity", alpha=0.5) +
  ...
  ggsave(paste0("clean_barplot", c,".png"), bg = 'white', width = 12, height = 12, units = c("in"))
  write.csv(data, paste0("radarplot_input_percity", c, ".csv")) #exporting individual subsets for data QA
  }


####Graphing As Radar Plots#### #02/03/2024 DEPRECATED - team opted for simpler bar charts
# for(c in cities){
#   data = radarplot_input %>%
#     filter(City == c) %>%
#     mutate(category = factor(category))
